...
/* ── Progress bar ── */
@keyframes evpls{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.3;transform:scale(.5)}}
...
// Real geographic coordinates — pins rotate WITH the globe
var hss=[
  {lat:49.0,  lon:32.0,  col:'#ff2d55', lb:'UKRAINE',  info:'Active frontline — eastern offensive'},
  {lat:31.5,  lon:34.45, col:'#ff2d55', lb:'GAZA',     info:'Ground operations ongoing'},
  {lat:32.0,  lon:53.0,  col:'#f0a500', lb:'IRAN',     info:'Post-strike nuclear posture elevated'},
  {lat:15.5,  lon:32.5,  col:'#f0a500', lb:'SUDAN',    info:'RSF advancing — famine declared'},
  {lat:17.0,  lon:96.0,  col:'#f0a500', lb:'MYANMAR',  info:'Junta vs resistance — 2M displaced'},
  {lat:33.9,  lon:35.5,  col:'#f0a500', lb:'LEBANON',  info:'Cross-border fire continuing'},
  {lat:15.4,  lon:44.2,  col:'#f0a500', lb:'YEMEN',    info:'Houthi attacks on Red Sea shipping'},
  {lat:34.0,  lon:66.0,  col:'#a855f7', lb:'AFGHAN',   info:'Taliban consolidation — aid cut'},
  {lat:18.9,  lon:-72.3, col:'#00d4ff', lb:'HAITI',    info:'Gang control — UN mission active'},
  {lat:6.5,   lon:20.5,  col:'#f0a500', lb:'C.AFR',    info:'Armed group activity continuing'},
];

// Continent outlines — lat/lon arrays projected onto globe
var CONTINENTS=[
  // Europe
  [[71,28],[70,31],[69,33],[67,14],[65,14],[63,8],[61,5],[59,5],[58,8],[55,8],[54,10],[54,18],[52,14],[51,4],[50,2],[48,2],[46,2],[43,7],[42,2],[40,2],[38,9],[36,5],[36,-5],[35,-6],[36,-5],[38,9],[40,18],[42,19],[41,20],[40,26],[38,27],[36,36]],
  // Africa
  [[37,10],[37,36],[30,32],[22,37],[12,43],[4,42],[0,42],[-4,40],[-10,38],[-22,35],[-30,30],[-34,26],[-34,18],[-28,17],[-22,14],[-18,12],[-14,12],[-5,10],[0,9],[4,7],[5,1],[3,9],[5,-5],[2,-9],[-5,-12],[-10,-14],[-15,-12],[-18,-13],[-22,-14],[-18,12]],
  // Asia (simplified)
  [[70,60],[70,90],[70,140],[60,140],[55,135],[43,141],[40,130],[35,130],[30,122],[25,120],[20,110],[15,108],[10,105],[5,103],[1,104],[-5,105],[10,77],[8,80],[12,80],[20,86],[22,92],[25,90],[27,89],[26,88],[22,92],[20,92],[15,100],[10,99],[5,101],[12,98],[16,97],[20,93],[25,87],[27,89],[30,78],[35,76],[40,74],[45,60],[50,53],[55,40],[60,38],[65,38],[70,30]],
  // North America
  [[70,-140],[71,-130],[72,-125],[73,-95],[72,-80],[70,-70],[68,-52],[60,-65],[55,-60],[50,-55],[44,-66],[42,-70],[40,-74],[38,-75],[35,-76],[32,-80],[25,-80],[24,-82],[18,-66],[10,-85],[8,-77],[15,-88],[18,-88],[22,-86],[24,-84],[28,-82],[30,-82],[32,-80],[38,-75],[43,-70],[47,-53],[55,-60],[59,-64],[65,-52],[70,-55],[70,-65],[71,-85],[73,-95],[72,-125]],
  // South America
  [[10,-62],[10,-75],[0,-80],[-5,-81],[-10,-77],[-18,-70],[-22,-70],[-30,-71],[-34,-71],[-42,-73],[-50,-75],[-55,-68],[-55,-65],[-52,-58],[-50,-50],[-40,-40],[-30,-38],[-22,-41],[-15,-39],[-8,-35],[-5,-35],[0,-50],[-5,-50],[0,-42],[5,-53],[8,-60],[10,-62]],
  // Australia
  [[-14,135],[-15,130],[-15,124],[-22,114],[-32,115],[-38,140],[-38,147],[-35,150],[-28,153],[-20,148],[-16,145],[-14,144],[-12,136],[-14,135]],
];
...
function drawGlobe(){
  ...
  CONTINENTS.forEach(function(pts){
    ...
    for(var i=0;i<pts.length;i++){
      var cp=ll(pts[i][0],pts[i][1],r,cx,cy);
      if(cp.z<0.08){started=false;continue;}
      if(!started){gbX.moveTo(cp.x,cp.y);started=true;}
      else gbX.lineTo(cp.x,cp.y);
    }
    gbX.strokeStyle='rgba(0,212,255,.16)';gbX.lineWidth=.7;gbX.stroke();
    gbX.fillStyle='rgba(0,212,255,.012)';gbX.fill();
  });

  // Conflict hotspot pulses — pinned to real lat/lon, rotate with globe
  var now=Date.now();
  hss.forEach(function(h,idx){
    var p=ll(h.lat,h.lon,r,cx,cy);
    if(p.z<0.08)return; // behind globe
    var pulse=(Math.sin(now*.003+idx*.9)+1)*.5;
    var hx=h.col.replace('#','');
    var rc=parseInt(hx.slice(0,2),16),gc=parseInt(hx.slice(2,4),16),bc=parseInt(hx.slice(4,6),16);
    // Glow halo
    var glo=gbX.createRadialGradient(p.x,p.y,0,p.x,p.y,14);
    glo.addColorStop(0,'rgba('+rc+','+gc+','+bc+','+(0.2+pulse*.15)+')');
    glo.addColorStop(1,'rgba('+rc+','+gc+','+bc+',0)');
    gbX.beginPath();gbX.arc(p.x,p.y,14,0,Math.PI*2);gbX.fillStyle=glo;gbX.fill();
    // Outer pulsing ring
    gbX.beginPath();gbX.arc(p.x,p.y,7+pulse*7,0,Math.PI*2);
    gbX.strokeStyle='rgba('+rc+','+gc+','+bc+','+(0.35*(1-pulse*.7))+')';
    gbX.lineWidth=1.2;gbX.stroke();
    // Inner ring
    gbX.beginPath();gbX.arc(p.x,p.y,3.5,0,Math.PI*2);
    gbX.strokeStyle='rgba('+rc+','+gc+','+bc+',.65)';gbX.lineWidth=1;gbX.stroke();
    // Core dot
    gbX.beginPath();gbX.arc(p.x,p.y,2.2,0,Math.PI*2);gbX.fillStyle=h.col;gbX.fill();
    // Label with connector tick
    var side=p.x>cx?1:-1;
    var lx=p.x+side*14;
    gbX.font='600 '+Math.round(W*.0055+5)+'px "IBM Plex Mono",monospace';
    gbX.textAlign=side>0?'left':'right';gbX.textBaseline='middle';
    gbX.fillStyle='rgba(194,212,238,.75)';
    gbX.fillText(h.lb,lx,p.y);
    // tick
    gbX.beginPath();gbX.moveTo(p.x+side*3,p.y);gbX.lineTo(lx-side*2,p.y);
    gbX.strokeStyle='rgba('+rc+','+gc+','+bc+',.3)';gbX.lineWidth=.6;gbX.stroke();
  });
}
...
function drawFX(){
  ...
  // Data arcs from projected hotspot positions to globe centre
  var sA=now*.00085,r2=Math.min(W,H)*.26;
  hss.forEach(function(h,idx){
    var ph=(sA+idx*1.05)%(Math.PI*2);if(ph>Math.PI)return;
    var pg=ph/Math.PI;
    var sp=ll(h.lat,h.lon,r2,cx,cy);
    if(sp.z<0.08)return;
    var mx=(sp.x+cx)*.5+18*Math.sin(idx*1.3),my=(sp.y+cy)*.5-48;
    fxX.beginPath();fxX.moveTo(sp.x,sp.y);
    fxX.quadraticCurveTo(mx,my,sp.x+(cx-sp.x)*pg,sp.y+(cy-sp.y)*pg);
    var hxc=h.col.replace('#','');
    fxX.strokeStyle='rgba('+parseInt(hxc.slice(0,2),16)+','+parseInt(hxc.slice(2,4),16)+','+parseInt(hxc.slice(4,6),16)+','+(Math.sin(pg*Math.PI)*.28)+')';
...
// ── Live event fetch (GDELT) ──────────────────────────────────
var staticEvts=[
  {col:'#ff2d55',tag:'UKRAINE', txt:'Russian missile strike on Kharkiv power infrastructure'},
  {col:'#ff2d55',tag:'GAZA',    txt:'IDF ground forces advance in northern corridor'},
  {col:'#f0a500',tag:'RED SEA', txt:'Houthi anti-ship missile intercepted near Bab el-Mandeb'},
  {col:'#f0a500',tag:'IRAN',    txt:'IRGC elevates ballistic missile readiness posture'},
  {col:'#f0a500',tag:'SUDAN',   txt:'UN declares famine in North Darfur — 750K at risk'},
];
function buildEventHTML(evts){
  return evts.slice(0,5).map(function(e){
    return '<div style="display:flex;align-items:flex-start;gap:7px;margin-bottom:5px">'
      +'<div style="width:6px;height:6px;border-radius:50%;background:'+e.col
      +';flex-shrink:0;margin-top:4px;animation:evpls '+(1+Math.random()*.8).toFixed(1)+'s ease-in-out infinite"></div>'
      +'<div style="font-size:clamp(8px,.85vw,10px);color:rgba(194,212,238,.55);line-height:1.5">'
      +'<span style="color:'+e.col+';font-weight:600;letter-spacing:.14em;margin-right:5px">'+e.tag+'</span>'
      +e.txt+'</div></div>';
  }).join('');
}
function fetchAndUpdateEvents(){
  ...
  if(!el)return;
  fetch('https://api.gdeltproject.org/api/v2/doc/doc?query=war+conflict+military+attack&mode=artlist&maxrecords=8&format=json&timespan=2h',
    {signal:AbortSignal.timeout(4000)})
    .then(function(r){return r.json();})
    .then(function(d){
      if(d&&d.articles&&d.articles.length){
        var evts=d.articles.slice(0,5).map(function(a){
          var t=a.title||'',tl=t.toLowerCase();
          var tag='WORLD',col='#00d4ff';
          if(tl.indexOf('ukraine')>=0||tl.indexOf('russia')>=0){tag='UKRAINE';col='#ff2d55';}
          else if(tl.indexOf('gaza')>=0||tl.indexOf('israel')>=0){tag='GAZA';col='#ff2d55';}
          else if(tl.indexOf('iran')>=0){tag='IRAN';col='#f0a500';}
          else if(tl.indexOf('houthi')>=0||tl.indexOf('red sea')>=0){tag='RED SEA';col='#f0a500';}
          else if(tl.indexOf('sudan')>=0){tag='SUDAN';col='#f0a500';}
          else if(tl.indexOf('myanmar')>=0){tag='MYANMAR';col='#f0a500';}
          return{col:col,tag:tag,txt:t.slice(0,80)};
        });
        el.innerHTML=buildEventHTML(evts);
        // Update ticker with live headlines
        var tk=document.getElementById('live-ticker');
        if(tk){
          var txt=evts.map(function(e){return'&#x25C6;&nbsp;'+e.tag+': '+e.txt+'&nbsp;&nbsp;&nbsp;';}).join('');
          tk.innerHTML=txt+txt;
        }
        // Mark feeds as live
        var fd=document.getElementById('feed-status');
        if(fd){fd.textContent='12 / 12 LIVE';fd.style.color='#00e676';}
      } else { if(el)el.innerHTML=buildEventHTML(staticEvts); }
    })
    .catch(function(){if(el)el.innerHTML=buildEventHTML(staticEvts);});
}
fetchAndUpdateEvents();
setInterval(fetchAndUpdateEvents,30000);
